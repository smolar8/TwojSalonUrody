...
function Podologia() {
  return (
    <div className="App">
      ...
      <br />
      {Podologia &&
        Podologia.map((podologia) => {
          return (
            <div className="box" key={podologia.id}>
              <strong>{podologia.title}</strong> <br />
              {podologia.text} <br> </br>
              {podologia.textBtn &&
                podologia.textBtn.map((data) => {
                  return <div key={podologia.id}>-- {data.text} --</div>;
                })}
            </div>
          );
        })}
